  ...
  <section className={`${styles.flexCenter} ${styles.paddingY}`}>
    ...
    flex-col mb-8 w-full`}>
      ...
      mr-10">
        ...
        max-w-[310px]`}>A new site with best design patterns</p>
      </div>

      <div className="flex-[1.5] w-full flex-row
      justify-between flex-wrap md:mt-0 mt-10">
        {footerLinks.map((footerLink) => (
          <div key={footerLink.key} className="flex
          flex-col ss:my-0 my-4 min-w-[150px]">
            <h4 className="front-poppins front-medium text-[18px] 
            ...
            <ul className="list-none mt-4">
                {footerLink.links.map((Link, index) => (
                  <li key={Link.name} 
                  className={`front-poppins front-normal text-[16px]
    ...
    md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3F3E43]">
      <p className="front-poppins front-normal text-center text-[18px] 
            leading-[27px] text-white">
        2022 GudV_Shop. All Rights Reserved.
      </p>
    </div>
